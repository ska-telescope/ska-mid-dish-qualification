...
def configure_logging(default_log_level: int = logging.INFO) -> None:
    ...
    if os.path.exists(disq_log_config_file):
        with open(disq_log_config_file, "rt") as f:
            try:
                ...
                print(e)
                print(
                    f"WARNING: Unable to read logging configuration file {disq_log_config_file}"
                )
    else:
    ...
    if config is None:
        ...
        try:
            ...
            print(e)
            print(
                f"WARNING: Caught exception. Unable to configure logging from file {disq_log_config_file}. Reverting to logging to the console (basicConfig)."
            )
            logging.basicConfig(level=default_log_level)
...
class scu:
    ...
    *** Exception caught
    ...
    def __init__(
        ...
        try:
            self.connection = self.connect(
                self.host, self.port, self.endpoint, self.timeout, encryption=True
            )
        except:
            try:
                self.connection = self.connect(
    ...
    def _enum_fields_out_of_order(
        ...
        logger.error(
            f"Incorrect index for field {field.Name} of enumeration {enum_name}. Expected: {index}, actual: {field.Value}"
        )
    ...
    def subscribe(
        ...
        if len(invalid_attributes) > 0:
            logger.warning(
                f"The following OPC-UA attributes not found in nodes "
    ...
    def unsubscribe(self, uid: int) -> None:
        ...
        _ = asyncio.run_coroutine_threadsafe(
            subscription["subscription"].unsubscribe(subscription["handle"]),
            self.event_loop,
        ).result()
    ...
    def abs_azel(
        ...
        if az_velocity is None or el_velocity is None:
            ...
            return
        logger.info(
            f"abs az: {az_angle:.4f} el: {el_angle:.4f}, az velocity: {az_velocity:.4f}, el velocity: {el_velocity:.4f}"
        )
