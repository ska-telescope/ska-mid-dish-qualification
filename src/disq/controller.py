...
class Controller(QObject):
    ...
    def command_response_str(self, command:str, return_code:int, return_msg:str) -> str:
    ...
    async def connect_server(self, server_uri):
        ...
        try:
            ...
            self.command_response_status.emit(f"Unable to connect to server. Error: {e}")
            return
    ...
    async def command_slew2abs(self,
                         azimuth_position:float, elevation_position:float,
                         azimuth_velocity:float, elevation_velocity:float):
        print("Command: slew2abs")
        cmd = "Slew2AbsAzEl"
        self.command_response_status.emit(f"Command: \"{cmd}\"...")
        retcode, retmsg = await self.model.call_method("Management", cmd,
                                                    azimuth_position, elevation_position,
                                                    azimuth_velocity, elevation_velocity)
