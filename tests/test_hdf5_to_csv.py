...
def test_start_stop_past_file(capsys, base_path):
    ...
    output_file = (
        base_path / "resources/output_files/start_stop_past_files.csv"
    )
    expected_file = (
        base_path / "resources/expected_files/start_stop_past_files.csv"
    )
    nodes = ["MockData.increment", "MockData.bool", "MockData.enum"]
    fo = h5py.File(input_file, "r")
    # Cause the requested start and end times to be past the file ranges
    start = datetime.fromisoformat(fo.attrs["Data start time"]) - timedelta(
        seconds=4
    )
    stop = datetime.fromisoformat(fo.attrs["Data stop time"]) + timedelta(
        seconds=4
    )
    fo.close()
    step = 100
    ...
    captured = capsys.readouterr()
    expected_stdout = (
        "Requested start time 2023-10-30 16:14:52.138941+00:00 is before earliest "
        "file start 2023-10-30 16:14:56.138941+00:00. Output CSV will start from "
        "2023-10-30 16:14:56.138941+00:00\nRequested stop time 2023-10-30 "
...
def test_simple_input_file(capsys, base_path):
    ...
    expected_file = (
        base_path / "resources/expected_files/simple_input_file.csv"
    )
    nodes = ["MockData.increment", "MockData.bool", "MockData.enum"]
    fo = h5py.File(input_file, "r")
    start = datetime.fromisoformat(fo.attrs["Data start time"])
...
def test_simple_input_file_double_speed(capsys, base_path):
    ...
    """
    input_file = (
        base_path / "resources/input_files/simple_input_file_double_speed.hdf5"
    )
    output_file = (
    ...
    expected_file = (
        base_path
        / "resources/expected_files/simple_input_file_double_speed.csv"
    )
    nodes = ["MockData.increment", "MockData.bool", "MockData.enum"]
    fo = h5py.File(input_file, "r")
    start = datetime.fromisoformat(fo.attrs["Data start time"])
